...
type Book = { id: string; title: string; author: string; subject: string; year: number; type: string; isbn: string; cover: string; coverImage?: string };
type Resource = { id: string; title: string; text: string; type: string; icon: typeof Library; tone: string };

const books: Book[] = [
  { id: "cien-anos", title: "Cien años de soledad", author: "Gabriel García Márquez", subject: "Narrativa", year: 1967, type: "Libro", isbn: "978-0307474728", cover: "terracotta", coverImage: "/images/books/cien-anos.svg" },
  { id: "pedagogia", title: "Pedagogía del oprimido", author: "Paulo Freire", subject: "Educación", year: 1970, type: "Libro", isbn: "978-8571140264", cover: "blue", coverImage: "/images/books/pedagogia.svg" },
  ...
  { id: "metodologia", title: "Metodología de la investigación", author: "Roberto Hernández Sampieri", subject: "Investigación", year: 2018, type: "Libro", isbn: "978-1456260871", cover: "blue", coverImage: "/images/books/metodologia.svg" },
...
function BookCover({ book }: { book: Book }) {
  ...
  return (
    <div className={`catalog-cover ${book.cover} ${hasImage ? "has-image" : "is-fallback"}`}>
      {hasImage && <img src={book.coverImage} alt={`Portada de ${book.title}`} onError={() => setImageFailed(true)} />}
      <span className="cover-type">{book.type}</span>
      {!hasImage && <BookOpen size={30} aria-hidden="true" />}
      <small>{hasImage ? "Portada" : "Portada no disponible"}</small>
    </div>
  );
}
